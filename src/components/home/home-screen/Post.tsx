...
const Post = ({ post, author, currentUser, isSubscribed }: { post: PostWithComments; author: User; currentUser: User; isSubscribed: boolean }) => {
  ...
  useEffect(() => {
    if (post.likesList && currentUser?.id) setIsLiked(post.likesList.some(like => like.userId === currentUser.id));
  }, [post.likesList, currentUser?.id]);
    ...
    <div className='flex flex-col gap-3 p-3 border-t'>
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <span className='font-semibold text-sm md:text-md'>{author.name}</span>
        </div>
